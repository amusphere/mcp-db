...
async function testMariaDB() {
  ...
  const dbUrl = process.env.MARIADB_URL || "mariadb://mcp:password@localhost:3307/mcp";
  console.log(`Connecting to: ${dbUrl.replace(/:[^:@]+@/, ':***@')}\n`);

  const config = normalizeDatabaseUrl(dbUrl);

  try {
    // Test 1: URL Normalization (MariaDB should be converted to MySQL)
    try {
      if (config.driver !== "mysql") {
        throw new Error(`Expected driver 'mysql', got '${config.driver}'`);
      }
      if (!config.connectionString.startsWith("mysql://")) {
        throw new Error("MariaDB URL should be converted to mysql://");
      }
      logTest("URL normalization (mariadb:// → mysql://)", true);
    } catch (error) {
      logTest("URL normalization (mariadb:// → mysql://)", false, (error as Error).message);
    }
    ...
    try {
      await executeSql(
        config,
        "CREATE TABLE test_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100), email VARCHAR(100))",
        undefined,
        false,
        500
      );
    ...
    // Test 6: List tables (MariaDB returns schema.table format like MySQL)
    try {
    ...
    // Test 7: Describe table (MariaDB needs schema specified)
    try {
    ...
    // Test 9: SELECT with named parameters
    try {
      const result = await executeSql(
        config,
