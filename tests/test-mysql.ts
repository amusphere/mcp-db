 ...
 * Tests basic operations against a MySQL database
 */
...
async function testMySQL() {
  ...
  const dbUrl = process.env.MYSQL_URL || "mysql://mcp:password@localhost:3306/mcp";
  ...
  try {
    ...
    try {
      const tables = await listTables(config, "mcp");
    ...
    // Test 16: Test MySQL backtick identifiers
    try {
      const result = await executeSql(config, "SELECT * FROM `test_users` WHERE `name` = :name", { name: "Alice" }, true, 500);
      if ("rows" in result && result.rows.length === 1) {
        logTest("MySQL backtick identifiers", true);
      } else {
        logTest("MySQL backtick identifiers", false, "Expected 1 row");
      }
    } catch (error) {
      logTest("MySQL backtick identifiers", false, (error as Error).message);
    }
